...
int main()
{
    ...
    std::cout << "With pipe operator : " << std::endl;
    auto my_view_with_pipe = vi | std::views::filter(even) | std::views::transform([](auto n)
                                                                                   { return n *= n; });

    std::cout << "vi transformed : ";
    print(my_view_with_pipe);
    ...
    // Print names in reverse order : this doesn't work if you store the data in an
